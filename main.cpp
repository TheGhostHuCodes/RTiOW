...
hitable* random_scene() {
    ...
    for (int a = -11; a < 11; a++) {
        for (int b = -11; b < 11; b++) {
            ...
            vec3 center(a + 0.9 * distribution(generator), 0.2,
                        b + 0.9 * distribution(generator));
            if ((center - vec3(4, 0.2, 0)).length() > 0.9) {
                if (choose_mat < 0.8) { // diffuse
                    ...
                    list[i++] = new sphere(
                        center, 0.2,
                        new metal(vec3(0.5 * (1 + distribution(generator)),
                                       0.5 * (1 + distribution(generator)),
                                       0.5 * (1 + distribution(generator))),
                                  0.5 * distribution(generator)));
                } else { // glass
